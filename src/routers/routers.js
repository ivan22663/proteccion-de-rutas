...
router.beforeEach((to, from, next) => {
  ...
  // Si la ruta existe en el sistema
  const routeExists = routes.some(route => {
    if (route.path === to.path) return true
    if (route.children) {
      return route.children.some(child => {
        const fullPath = route.path + '/' + child.path
        return fullPath === to.path || to.name === child.name
      })
    }
    return false
  })

  // Si la ruta no existe, redirigir a NotFound
  if (!routeExists && to.name !== 'NotFound' && to.path !== '/:pathMatch(.*)*') {
    next({ name: 'NotFound' })
    return
  }
  ...
  if (to.meta.requiresAuth) {
    ...
    // Si la ruta tiene roles específicos, verificar autorización
    if (to.meta.roles && to.meta.roles.length > 0) {
      // Si no tiene rol o el rol no está en la lista permitida
    ...
    // Rutas públicas (login, recuperar contraseña, etc.)
    next()
  }
})
